...
function savePersistantValue($persistance, $db_key, $field_name, $field_value) {
	...
	$params = array (
			...
			( string ) $field_name,
			( string ) $value,
			( string ) $_SESSION['entry_key_practiceexam-tutorial'],
			( string ) $remote_ip 
	);
...
function savePersistantValues($persistance, $db_key, $resume_mode, $on_success) {
	...
	foreach ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] as $key => $value ) {
		
		foreach($triggers as $t){
			
			if(isset($_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$key])) {
				...
				if (in_array ( $key, $keys )) {
					...
					if($is_file_upload_data && $field_value != null) {
						...
						$params = array (
								...
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$key}_mime"], // file_mime
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$key}_name"], // file_name
								( string ) $key,
								( int ) $db_key,
								( string ) $remote_ip
						);
					...
					if($is_file_upload_data && $field_value != null) {
						...
						$params = array (
								...
								( int ) $timestamp,
								( string ) $key, // name
								( string ) $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$key], // label
								( string ) '', // entry value
								$field_value, // file data
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$key}_mime"], // file_mime
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$key}_name"], // file_name
								( string ) $_SESSION['entry_key_practiceexam-tutorial'], // entry_key
						...
						$params = array (
								...
								( int ) $timestamp,
								( string ) $key,
								( string ) $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$key],
								( string ) $field_value,
								( string ) $_SESSION['entry_key_practiceexam-tutorial'],
								( string ) $remote_ip
						);
	...
	if(isset($_POST['rf_save_and_return']) && $_POST['rf_save_and_return'] == "true" && isset ( $_SESSION ['fb_entry_id_auto'] )){
		...
		if(isset($_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_LANGUAGE'])){
			$persistancelanguage = (int)$_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_LANGUAGE'];
		}
		...
		if($allow_email){
			...
			if(!isset($_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_EMAIL_LINK']))
				$_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_EMAIL_LINK'] = 0;
...
EOF;
			...
			if(isset($_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_EMAIL_LINK']) && $_SESSION["{$_SESSION['entry_key_practiceexam-tutorial']}"]['SAVE_AND_RESUME_EMAIL_LINK'] >= 3){
		...
		foreach ( $entry_rows as $row ) {
			...
			if ($row ['name'] == 'FB_PAGE_ACCESS') {
				...
				foreach ( $pages as $p ) {
					...
					$_SESSION['pages-passed']["{$_SESSION['entry_key_practiceexam-tutorial']}"]["{$p}"] = 'pass'; // Build 877
				}
			} elseif ($row ['name'] == 'signatures') {
				// Restore signature data.
				$_SESSION['qs']["{$_SESSION['entry_key_practiceexam-tutorial']}"]["signatures"] = unserialize($row['sig_data']);
			} else {
...
function replaceEcommPriceTokens($code) {
	...
	if (isset ( $matches [3] )) {
		foreach ( $matches [3] as $idx => $m ) {
			$code = isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$m}"] ) ? $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ["{$m}"] : '';
...
function clear_session() {
	...
	if(!isset($_SESSION['entry_key_practiceexam-tutorial']))
		return;
	
	if(!isset($_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"]))
		return;
		
		// Remove all singleton session data fields (selected items etc)
	$named_sesison_vars = array_keys ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] );
	...
	if (isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ['signatures'] )) {
		unset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] ['signatures'] );
	}
	
	unset ( $_SESSION ['pages'] );
	unset ( $_SESSION ['pages-passed'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] );
	
	clean_output_location ( 'tmp' );
	clean_output_location ( 'lib/jquery-upload/server/php/files/' . $_SESSION['entry_key_practiceexam-tutorial'] ); // Build 860
	
	if (isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] )) {
		unset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] );
	}
	if (isset ( $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] )) {
		unset ( $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] );
	}
	if (isset ( $_SESSION['entry_key_practiceexam-tutorial'] )) {
		unset ( $_SESSION['entry_key_practiceexam-tutorial'] );
	}
...
if (! function_exists ( "process_array_variables" )) {
	function process_array_variables($vars, $separator) {
		...
		foreach ( $vars as $var ) {
			if (isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$var] )) {
				if (is_array ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$var] )) {
					$field_items = '';
					foreach ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$var] as $key => $v ) {
						if ($key != 0) {
							$field_items .= $separator;
						}
						$field_items .= $v;
					}
					$_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-tutorial']}"] [$var] = $field_items;
				}
			}
		}
	}
