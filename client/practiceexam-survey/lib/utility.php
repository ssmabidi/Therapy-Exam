...
function savePersistantValue($persistance, $db_key, $field_name, $field_value) {
	...
	$params = array (
			...
			( string ) $field_name,
			( string ) $value,
			( string ) $_SESSION['entry_key_practiceexam-survey'],
			( string ) $remote_ip 
	);
...
function savePersistantValues($persistance, $db_key, $resume_mode, $on_success) {
	...
	foreach ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] as $key => $value ) {
		...
		foreach($triggers as $t){
			...
			if(isset($_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-survey']}"] [$key])) {
				...
				if (in_array ( $key, $keys )) {
					...
					if($is_file_upload_data && $field_value != null) {
						...
						$params = array (
								...
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$key}_mime"], // file_mime
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$key}_name"], // file_name
								( string ) $key,
								( int ) $db_key,
								( string ) $remote_ip
						);
					...
					if($is_file_upload_data && $field_value != null) {
						...
						$params = array (
								...
								( int ) $timestamp,
								( string ) $key, // name
								( string ) $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-survey']}"] [$key], // label
								( string ) '', // entry value
								$field_value, // file data
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$key}_mime"], // file_mime
								( string ) $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$key}_name"], // file_name
								( string ) $_SESSION['entry_key_practiceexam-survey'], // entry_key
						...
						$params = array (
								...
								( int ) $timestamp,
								( string ) $key,
								( string ) $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-survey']}"] [$key],
								( string ) $field_value,
								( string ) $_SESSION['entry_key_practiceexam-survey'],
								( string ) $remote_ip
						);
	...
	if(isset($_POST['rf_save_and_return']) && $_POST['rf_save_and_return'] == "true" && isset ( $_SESSION ['fb_entry_id_auto'] )){
		...
		if(isset($_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_LANGUAGE'])){
			$persistancelanguage = (int)$_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_LANGUAGE'];
		}
		...
		if($allow_email){
			...
			if(!isset($_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_EMAIL_LINK']))
				$_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_EMAIL_LINK'] = 0;
...
EOF;
			...
			if(isset($_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_EMAIL_LINK']) && $_SESSION["{$_SESSION['entry_key_practiceexam-survey']}"]['SAVE_AND_RESUME_EMAIL_LINK'] >= 3){
		...
		foreach ( $entry_rows as $row ) {
			...
			if ($row ['name'] == 'FB_PAGE_ACCESS') {
				...
				foreach ( $pages as $p ) {
					...
					$_SESSION['pages-passed']["{$_SESSION['entry_key_practiceexam-survey']}"]["{$p}"] = 'pass'; // Build 877
				}
			} elseif ($row ['name'] == 'signatures') {
				// Restore signature data.
				$_SESSION['qs']["{$_SESSION['entry_key_practiceexam-survey']}"]["signatures"] = unserialize($row['sig_data']);
			} else {
...
function replaceEcommPriceTokens($code) {
	...
	if (isset ( $matches [3] )) {
		foreach ( $matches [3] as $idx => $m ) {
			$code = isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$m}"] ) ? $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] ["{$m}"] : '';
...
function clear_session() {
	...
	if(!isset($_SESSION['entry_key_practiceexam-survey']))
		return;
	...
	clean_output_location ( 'lib/jquery-upload/server/php/files/' . $_SESSION['entry_key_practiceexam-survey'] ); // Build 860
	
	if (isset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] )) {
		unset ( $_SESSION ['qs'] ["{$_SESSION['entry_key_practiceexam-survey']}"] );
	}
	if (isset ( $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-survey']}"] )) {
		unset ( $_SESSION ['qs-label'] ["{$_SESSION['entry_key_practiceexam-survey']}"] );
	}
	if (isset ( $_SESSION['entry_key_practiceexam-survey'] )) {
